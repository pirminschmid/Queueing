#!/bin/sh
# controls experiment 3.2 two middlewares
# 3 memtier VM, 2 middleware, 1 memcached server
# assumes vm 1,2,3,4,5,6 running
# write only
# read only
# 1 min stable each
# min 3 repetitions
...
. ./hello_middlewares.source
. ./hello_s6.source
echo "\nprepare VMs: clean data folder (c1, c2, c3; m4, m5; and s1=s6 must succeed)"
pssh -h clients_middlewares.hostfile -H $s6 "mkdir -p $data_path && rm -rf $data_path/*"

# run iperf
. ./100_run_iperf_3c_2mw_1s.source
...
pssh -h middlewares.hostfile -t 0 "sh $run_path/run_ping.all.sh $r $ping_size s1 $s6" &


# launch dstat
echo "\nstart dstat"
pssh -h clients_middlewares.hostfile -H $s6 -t 0 "sh $run_path/run_dstat.all.sh $r" &
...
# stop dstat
echo "\nstop dstat"
pssh -h clients_middlewares.hostfile -H $s6 "killall dstat"
