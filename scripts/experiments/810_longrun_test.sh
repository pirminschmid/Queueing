#!/bin/sh
# controls additional experiment 8.1 long run of the system
# 3 memtier VM, 2 middlewares, 3 memcached servers
# assumes vm 1,2,3,4,5,6,7,8 running
# mixed workload with sharding (more load on middleware) with 9 keys in gets; experiment 510 was used as template
# with mt=wt=64 to have more complexity in middleware
# longer run as defined in run_experiments.sh: $memtier_runtime_longrun
# currenlty only 1 iteration here (for cost reasons)
# the question to be answered is mainly whether TP and RT remain approx the same over a long period of time
#
...
for iteration in 1 2 3 4; do
    ...
    ccmd="sh $run_path/run_memtier_two_instances.client.sh $exp_key $ratio $ct $cv $vm1 $vm2 $port $memtier_runtime_longrun"
