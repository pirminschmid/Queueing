...
if [ "$mode" = "par" ]; then
    ...
    iperf -c $ip3 >$data_path/r_${r}_i_${iteration}_app_iperf_id_${type}${id}${name3}-${mode}.iperf.data
    ...
    echo "\nrunning iperf ${type}${id} -> ${name3}"
    iperf -c $ip3 | tee $data_path/r_${r}_i_${iteration}_app_iperf_id_${type}${id}${name3}-${mode}.iperf.data
fi
